...
- `bookshelf_character_profile_get`：读取或创建当前作品的角色档案。
...
- `bookshelf_character_draft_upsert` / `bookshelf_character_draft_apply` / `bookshelf_character_draft_rollback`：草稿式角色卡流程，适合需要先审阅再应用的场景。
...
2. 用户问题能直接从已给上下文回答时，不必调用 MCP；需要真实书架数据时再调用只读工具。
...
如果上下文已经包含当前书，优先使用上下文；否则调用 `bookshelf_current_book_get`。需要目录或正文时再调用章节/正文工具。适合回答剧情、章节位置、缓存状态、正文异常、替换净化建议等。
