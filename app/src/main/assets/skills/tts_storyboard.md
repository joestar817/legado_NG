...
6. 证据不足以绑定到 `knownCharacters`，但可以确认这是人物对白/心声且可以确认说话人姓名时，对 `character` / `thought` 返回 `status="unknown"`，`characterId=0`，并把真实姓名填入 `characterName`。
7. 证据不足以绑定到 `knownCharacters`，也没有明确姓名，但可以确认说话人性别时，对 `character` / `thought` 返回 `status="unknown"`，`characterName=""`，并填写 `speakerGender` 为 `male` 或 `female`。
8. 如果连说话人性别也无法确认，返回 `narrator` 或 `other`，不要猜测。
9. 输出前做自检：字段、枚举、目标覆盖、无正文输出全部必须满足。
...
- `characterName`：命中 `knownCharacters` 时填写角色名；`character` / `thought` 且 `status="unknown"` 时，只有上下文给出了明确人物姓名或稳定称呼才填写；不确定、只有泛称、只有动作描述或无法抽出姓名时必须留空；`narrator` / `other` 必须为空字符串。
...
- 没命中 `knownCharacters`，但确认是男声/男心声：`roleType="character"` 或 `thought`，`status="unknown"`，`characterId=0`，`speakerGender="male"`；如果能明确抽出说话人姓名或稳定称呼，`characterName` 填该姓名；否则留空。
- 没命中 `knownCharacters`，但确认是女声/女心声：`roleType="character"` 或 `thought`，`status="unknown"`，`characterId=0`，`speakerGender="female"`；如果能明确抽出说话人姓名或稳定称呼，`characterName` 填该姓名；否则留空。
- 不能确认性别：不要输出 `character` / `thought`，改为 `narrator` 或 `other`，`speakerGender="unknown"`。

`unknown.characterName` 的边界：

- 只在能明确抽出说话人姓名或稳定称呼时填写。
- 不能确认姓名、只能确认性别时必须留空，客户端会显示 `对白男` / `对白女`。
- 不要把动作、语气、发言动词、证据短语、正文片段、泛称或占位描述写进 `characterName`。
...
- `character` / `thought` 且 `status="unknown"` 时，如果证据里有明确说话人姓名或稳定称呼，应该把 `characterName` 填为该姓名或称呼。
- `character` / `thought` 且 `status="unknown"` 时，如果只有泛称、声音描述或身份描述，不要强行编名字，`characterName` 留空，客户端会显示 `对白男` / `对白女`。
- `characterName` 只能是人物名或稳定称呼，不能是证据短语；如果不能抽出人物名就留空。
- 如果 `evidence` 中能写出明确说话人姓名，`characterName` 必须填同一个姓名，不能只把姓名写在 `evidence` 里。
...
10. `character` / `thought` 且 `status="unknown"` 时，如果 `characterName` 非空，必须是人物名或稳定称呼，不能是动作、语气、证据短语、正文片段、泛称或占位描述。
11. `character` / `thought` 且 `status="unknown"` 时，如果 `evidence` 中已经写出明确说话人姓名，`characterName` 必须填写同一个姓名；如果无法确定姓名，`evidence` 也不要写成姓名归因。
12. `allowNewCharacters=false` 时，`newCharacters` 必须是空数组。
...
第一遍：unknown 如果能明确识别真实人物名，应输出真实人物名。

第二遍：unknown 如果不能明确识别真实人物名，`characterName` 必须留空。

第三遍：unknown 的 `characterName` 绝不能是动作短语、证据短语或正文片段。
