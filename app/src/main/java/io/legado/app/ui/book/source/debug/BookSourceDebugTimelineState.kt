...
class BookSourceDebugTimelineState {

    private val logs = mutableListOf<RawLog>()
    private val responses = mutableMapOf<Int, String>()
    private var sourceName: String = ""
    var items: List<DebugItem> = emptyList()
        private set

    fun addLog(state: Int, msg: String): List<DebugItem> {
        logs.add(RawLog(state, msg, parseElapsedMs(msg)))
        return refreshItems()
    }

    fun addResponse(state: Int, msg: String): List<DebugItem> {
        responses[state] = msg
        return if (logs.isNotEmpty()) refreshItems() else items
    }

    fun setSourceName(name: String): List<DebugItem> {
        sourceName = name
        return if (logs.isNotEmpty()) refreshItems() else items
    }

    fun clearLogs(): List<DebugItem> {
        logs.clear()
        responses.clear()
        items = emptyList()
        return items
    ...
    private fun refreshItems(): List<DebugItem> {
        items = buildItems()
        return items
    }
    ...
    fun rawText(phaseId: String): String {
