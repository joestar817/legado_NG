...
/** Draws the opaque Sakura scene inside the normal Compose/View layer order. */
internal class ListeningSakuraTextureView(context: Context) :
    TextureView(context),
    ...
    @Volatile
    private var renderConfig = SakuraConfig()
    ...
    private inner class RenderSession(
        ...
        fun start() {
            ...
            handler.post {
                ...
                runCatching {
                    ...
                    check(egl.swapBuffers()) { "Unable to swap initial Sakura buffer" }
                    ...
                    active = false
                    Log.e(TAG, "Unable to start Sakura TextureView", error)
                    revealFallback()
                    finishSession()
                }
            }
            ...
            decoderHandler.post decode@{
                val textureData = runCatching {
                    ...
                    handler.post {
                        if (active) {
                            active = false
                            Log.e(TAG, "Unable to decode Sakura assets", error)
                            revealFallback()
                            finishSession()
                        }
                    }
                ...
                val accepted = handler.post upload@{
                    ...
                    runCatching { motionRenderer.uploadAssets(textureData) }
                        ...
                        .onFailure { error ->
                            active = false
                            Log.e(TAG, "Unable to upload Sakura assets", error)
                            revealFallback()
                            finishSession()
                        }
                }
        ...
        private fun renderFrame(frameTimeNanos: Long) {
            ...
            runCatching {
                ...
                check(eglWindow?.swapBuffers() == true) { "Unable to swap Sakura EGL buffers" }
            }.onFailure { error ->
                active = false
                Log.e(TAG, "Sakura TextureView frame failed", error)
                revealFallback()
                finishSession()
                return
            }
        ...
        private fun releaseOnRenderThread() {
            ...
            runCatching { motionRenderer.release() }
                .onFailure { Log.w(TAG, "Unable to release Sakura renderer", it) }
            runCatching { eglWindow?.release() }
                .onFailure { Log.w(TAG, "Unable to release Sakura EGL window", it) }
            eglWindow = null
        }
        ...
        private fun releaseOwnedSurfaceTexture() {
            if (surfaceReleased.compareAndSet(false, true)) {
                runCatching { surfaceTexture.release() }
                    .onFailure { Log.w(TAG, "Unable to release owned Sakura SurfaceTexture", it) }
            }
        }

        private fun revealFallback() {
            val failedSession = this
            this@ListeningSakuraTextureView.post {
                val stillCurrent = synchronized(sessionLock) { session === failedSession }
                if (stillCurrent) this@ListeningSakuraTextureView.alpha = 0f
            }
        }
    }
            ...
            fun create(surfaceTexture: SurfaceTexture): EglWindow {
                ...
                try {
                    ...
                    check(EGL14.eglMakeCurrent(display, surface, surface, context)) {
                        "Unable to make Sakura EGL context current"
                    }
    ...
    private companion object {
        const val TAG = "ListeningSakuraTextureView"
