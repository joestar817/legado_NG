...
internal object JsSourceDebugFormatter {
    ...
    fun chapterList(chapters: List<BookChapter>): List<String> = buildList {
        ...
        chapters.firstOrNull()?.let { addChapter("首章", it) }
        if (chapters.size > 1) addChapter("末章", chapters.last())
    }
    ...
    private fun formatValue(value: Any?): String {
        val sanitized = NetworkLog.redactFreeTextForLog(value?.toString().orEmpty())
