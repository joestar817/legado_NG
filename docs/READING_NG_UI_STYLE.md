...
- 整体偏暗的听书／音频播放页使用“暗色沉浸背景 + 高透近中性液态控制层”：关闭按钮、快捷圆钮、进度 Thumb、底部播放 Dock 和音色／来源胶囊可以使用实时折射；章节／字幕、歌词／音频信息卡、封面、标题和正文属于稳定内容层，不随视觉体系自动液态化。控制层只吸收极少封面主题色，以窄边缘折射、弱方向高光和按压形变表达液态，不增加明显灰青／主题色实体层，也不另采样未压暗封面形成亮色切口。
